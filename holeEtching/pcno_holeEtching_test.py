...
def load_data(data_path):
    ...
    for i in range(n_data):    
        for mesh_type in ["fixradius", "radius"]:
            data=np.load(data_path+"/"+mesh_type+"/data_%05d"%(i)+".npy")
            elems=[]
            
            for j in range(len(data)-1):
                if abs(data[j][0]-data[j+1][0])<0.1:
                    elems.append([1,j,j+1])
...
parser.add_argument('--train_distribution', type=str, default='uniform', choices=['fixradius', 'radius','mixed'],
                    help='distribution of training dataset (fixradius, radius)')

parser.add_argument('--n_train', type=int, default=1000, choices=[500, 1000, 1500],
                    help='training datasize (500,1000,1500)')
parser.add_argument('--n_test', type=int, default=600, help='Number of testing samples')
parser.add_argument('--equal_weight', type=str, default='False', help='Specify whether to use equal weight')

parser.add_argument('--train_inv_L_scale', type=str, default='False', choices=['False' ],
...
x_train, x_test = torch.cat((features[:n_train,:,[6,7,8,9,10,11,12]], nodes_input[:n_train,...], node_rhos[:n_train, ...]), -1), torch.cat((features[-n_test:,:,[6,7,8,9,10,11,12]],nodes_input[-n_test:,...], node_rhos[-n_test:, ...]), -1)
aux_train       = (node_mask[0:n_train,...], nodes[0:n_train,...], node_weights[0:n_train,...], directed_edges[0:n_train,...], edge_gradient_weights[0:n_train,...])
aux_test        = (node_mask[-n_test:,...],  nodes[-n_test:,...],  node_weights[-n_test:,...],  directed_edges[-n_test:,...],  edge_gradient_weights[-n_test:,...])

# y_train, y_test = features[:n_train, :, [0,1,2,3,4,5]],     features[-n_test:, :, [0,1,2,3,4,5]]
y_train, y_test = features[:n_train, :, [0,1,2,3,4,5]],     features[-n_test:, :, [0,1,2,3,4,5]]
...
train_rel_l2_losses, test_rel_l2_losses, test_l2_losses = PCNO_train(
    x_train, aux_train, y_train, x_test, aux_test, y_test, config, model, save_model_name=f"./model/PCNO_holeEtching_model_{train_distribution}_{n_train}"
)
